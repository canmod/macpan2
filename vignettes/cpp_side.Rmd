...
output: 
  rmarkdown::html_vignette:
    toc: true
vignette: >
  %\VignetteIndexEntry{The C++ Side of McMasterPandemic}
...
After the input matrices in `mats` are updated using `params` and `random`, the these matrices can be modified. We refer to this process of modification as trajectory simulation. There are three phases to the trajectory simulation process.
...
* `DATA_IVECTOR(expr_output_id)`
    ...
    * Matrices that are modified by the same function must be sequentially grouped together in `mats`
* `DATA_IVECTOR(expr_sim_block)`
    ...
    * Expressions in a simulation block must be grouped together
    ...
    * Note that if an expression is evaluated in simulation blocks and is returned to the user through `mats_return == 1`, then it must also be returned within a simulation block
...
Each expression is evaluated in the order in which it appears in this vectors.
...
Inputs are invalid if the sum of the elements of `eval_schedule` does not equal the number of elements in each of `expr_output_count`, `expr_output_id`, `expr_sim_block`, and `expr_num_p_table_rows`.
...
We use the following definition of element-wise binary operators.

* If both operands have the same shape, then do the element-wise operation
* If one of the operands is a scalar, then do the operation of scalar vs
  elements of the other operand
* If one of the operands is a vector, then
...
TODO: Consider adding a special function for 'running a simulation loop' so that we can do things like put priors on R0. The idea with this use case is that R0 is computed be simulating a special cohort model to produce the kernel, the sum of which is R0. Perhaps what we should do is to allow the user to run `n` simulation loops? Or perhaps another way around this is to add objective functions for two different simulation models.
...
For all four of these functions, if `m` does not refer to a matrix that has a saved simulation history than an error is thrown. Similarly, an error is thrown if any index is out of bounds (i.e. greater than `T+1` or less than `0`.
