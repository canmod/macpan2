...
```{r echo = FALSE}
StringDataFromDotted(models$seir_vax$settings()$state_variables, "Epi.Vax")$undot()$frame()
```
...
```{r, echo = FALSE}
Epi = models$seir$variables()  ## delete when no longer used
models$seir$variables()
```
