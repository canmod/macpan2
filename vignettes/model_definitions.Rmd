...
* `flow_type` -- A type describing the way in which the magnitude of flow is quantified. The following options are available.
    * `"per-capita"` -- The flow from `from` and to `to` equals `from * flow`.
    * `"absolute"` -- The flow from `from` and to `to` equals `flow`.
    * `"per-capita_inflow"` -- The flow to `to` equals `from * flow`.
    * `"per-capita_outflow"` -- The flow from `from` equals `from * flow`.
