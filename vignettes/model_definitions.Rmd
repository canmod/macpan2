...
A compartmental model is a named list of the following four components.
...
4. `settings` -- A named list of elements that determine how components 1-3 are used by the software.
...
[Variable Names] are used in lists that identify variables with specific roles in the model. There are two such lists in the `settings` component.
...
* `flow_component` -- The name of the variable governing the magnitude of the flow.
* `component_type` -- A type describing the way in which the magnitude of flow is quantified. The following options are available.
    * `"rate"` -- The flow from `from` and to `to` equals `from * flow_component`.
    * `"absolute"` -- The flow from `from` and to `to` equals `flow_component`.
    * `"rate_inflow"` -- The flow to `to` equals `from * flow_component`.
    * `"rate_outflow"` -- The flow from `from` equals `from * flow_component`.
    * `"absolute_inflow"` -- The flow to `to` equals `flow_component`.
    * `"absolute_outflow"` -- The flow from `from` equals `flow_component`.
...
## Compressed Data Format for Flows
