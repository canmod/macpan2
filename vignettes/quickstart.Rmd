...
Now that we have a directory that completely defines a compartmental model, we can use it to create a **model structure** object in R using the `Compartmental` function. (`Compartmental` is a function that initializes `Compartmental` objects, which are special to `macpan2`.)
