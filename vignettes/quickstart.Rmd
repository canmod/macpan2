...
(BB: do we want this? Start a `macpan_helpers` package?)

```{r visCompartmental, echo = FALSE}
#' Visualize a Compartmental model
#' 
#' @param model Model object created using [Compartmental()]
...
```{r make_visnet, echo = FALSE}
sir = Compartmental(sir_dir)
if (require(visNetwork, quietly = TRUE)) {
    vn = visCompartmental(sir)
    visSave(vn, file = paste0("visnet.html"))
}
```

```{r show_visnet, results = "asis", echo = FALSE, eval = TRUE}
## this fails in pkgdown build, suppressing for now ...
...
In this example, the derived variables are the total population size, `N`, and the force of infection, `foi`. Each derived variable is defined in a [JSON](https://en.wikipedia.org/wiki/JSON) object. Each such JSON object will contain several key-value pairs. Here the keys are:
...
Here, the flow type is `per_capita`: i.e., the flow at time step $t+1$ will equal `flow` times the number of individuals in the `from` compartment at time $t$. In the example above, the flow from `S` to `I` equals `foi * S` (referring back to the derivations file, this is equal to `beta * S * I/ N`).
