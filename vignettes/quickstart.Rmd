...
- make diagram for two-strain model and include it below
...
While a modeller could certainly write their own code to cut down on repetition when expanding a simple model (and many do), the idea behind `macpan2` is to give individuals a ready-made model specification grammar that enables easy model extension, especially when building [product models](https://arxiv.org/abs/2307.10308), and that can readily interface with blazing fast simulation engines, like [TMB](https://cran.r-project.org/web/packages/TMB/index.html). 
...
[IP: small point, but i assume the row index starts at 4 because the first three entries (where `time = 0`) are the initial conditions. it would be a little cleaner if those row names were removed before output from `mp_report()`.]
