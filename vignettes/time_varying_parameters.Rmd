...
1. `beta_changepoints` - An integer vector containing the starting time steps at which the tranmission rate `beta` changes. This vector starts with a time step of 0 because we want to specify an initial default value of `beta` followed by changing transmission rates at the beginning of time-step 10 and 15
...
We then need to specify an expression that updates `beta` to `beta_values` at the time steps in `beta_changepoints`. To do this we use the [`time_step`](https://canmod.github.io/macpan2/reference/engine_functions.html#time-indexing) function to get the current time step for each iteration in the loop. We specify a time lag of 0 from the current time step with `time_step(0)` because we want the value of the current time step. 

We then use the [`time_var`](https://canmod.github.io/macpan2/reference/engine_functions.html#time-indexing) function that will return a value for `beta`, from `beta_values`, by comparing the current time step with the time steps in `beta_changepoints`.
```{r time_var}
# see ?time_var for arguments
expr = list(
 beta ~ time_var(beta_values, beta_changepoints, time_step(0))
)
...
<!-- perhaps there is a new way to do this, should show this way-->
...
```{r transform}
mat_beta_values = (
  data.frame(value = beta_values)
  |> mutate(
    value = log(value),
    row = seq(1,length(beta_values),by=1),
    col = 0,
    matrix = "log_beta_values")
)
...
# make default appropriately
# a matrix for beta_values
...
#   , data = I_observed
#   , traj = "I"
#   , par = mat_beta_values # or beta_values
