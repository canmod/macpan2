...
Now we're going to try to calibrate this to data. The main innovation here is that we will use a 
...
$$
\begin{split}
I_{\textrm{obs}} & \sim \textrm{Normal}(I_\textrm{sim}(\beta, {\mathbf b}), \sigma^2_I) \\
b_i & \sim \textrm{Normal}(0, \sigma^2_{\textrm{rbf}})
\end{split}
$$
and the likelihood is defined as:
$$
\int {\cal L}(I_{\textrm{obs}}|\beta, {\mathbf b}', \sigma^2_I) \cdot {\cal L}({\mathbf b}'|\sigma^2_{\textrm{rbf}}) \, d{\mathbf b}.
$$

(In the example here $\beta$ is empty/null; note that in statistical notation it represents a *parameter vector*, not a transmission coefficient as is generally the case in epidemic models ...)
...
- the RBF function is penalized to zero. In general, we should augment the penalized RBF component (which determines the variation around the mean) with an unpenalized intercept/baseline transmission parameter. So, for example, the computation of the transmission coefficient should be `eta ~ gamma * (b0 + exp(X %*% b))`, where `b0` represents an unpenalized parameter that's allowed to vary freely ...
