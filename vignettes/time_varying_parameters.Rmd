...
```{r noisy_sim, fig.width = 8, fig.height = 4}
obs_I <- (
    simulator$report(.phases = "during")
...
$$
\begin{split}
I_{\textrm{obs}} & \sim \textrm{Normal}(I_\textrm{sim}(\beta, b), \sigma^2_I) \\
...
```{r rbf_test_objfun, results = "hide"}
simulator$ad_fun()$fn(c(1,1))
```
(this step produces more output than usual because the Laplace approximation involves an additional "inner" step where the `b` parameters are optimized, even though we are only evaluating the objective for a single set of fixed parameters (`log_I_sd`, `log_rbf_sd`)
...
```{r plot_sim, fig.width = 8, fig.height = 4}
