...
### computing and plotting confidence and prediction intervals (EXPERIMENTAL)
...
library(TMB)
library(tmbstan)
stan1 <- tmbstan(sir_simulator$ad_fun(), seed = 101)
```

```{r tmbparams, eval = require("broom.mixed")}
broom.mixed::tidy(stan1, conf.int = TRUE)
```

Can extract samples via e.g. `as.data.frame()`  (or `tidybayes` package), run ensembles, ...
