...
plot(obs_val ~ time, data = sir_prevalence)
with(sir_prevalence, lines(time, value))
```
...
that the simulation histories are saved;
...
**Fixme**: what are the `beta_*` things doing in there? Are they hard-coded somewhere? (Why?)
...
```{r}
simulator$insert$expressions(
  log_lik ~ dnorm(I_obs, rbind_time(I_sim), I_sd)m
  .phase = "after"
  )
```
