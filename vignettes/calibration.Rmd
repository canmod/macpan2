...
**fixme**: better transition explaining (1) that we are revealing more of the low-level details of `macpan2` (can we add an interface for this??); (2) that we are adding stuff to the `sir_simulator` object that wasn't needed when we only wanted to use it as a simulator; (3) that `$add$matrices()` is adding data objects to a space within the object that was initially populated from the "variables" and "expression" files
...
**fixme**: explain that, like `$add$matrices()`, this adds components that could have been included initially (in the "expressions" file)
...
**fixme**: explain that adding the Log transformation automatically adds a `log_*` variable/matrix
...
**fixme**: examples of (statistical) diagnostics?
