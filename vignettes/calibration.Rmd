...
```{r mk_calibrate}
macpan2::mk_calibrate(sir_simulator,
   data = data.frame(I_obs = sir_prevalence$obs_val),
   params = list(trans_rates = c(beta = 1), I_sd = 1),
   transforms = list(trans_rates = c(beta  = "log"), I_sd = "log"),
...
```{r repl_param}
sir_simulator$replace$params(c(0, 1), c("log_trans_rates", "log_I_sd"))
```
...
```{r rbf_model, eval=FALSE, echo=TRUE}
