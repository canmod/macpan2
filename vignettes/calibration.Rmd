...
**experimental** delta-method-based confidence intervals (requires most recent macpan2
...
res <- (cbind(sir_simulator$report(), sd = ss[,5])
...
We can also generate intervals based on multivariate normal sampling, which relaxes the second assumption but not the first.

```{r ci_ensemble}
set.seed(101)
nsim <- 1000
parvals <- MASS::mvrnorm(nsim, mu = sdr$par.fixed, Sigma = sdr$cov.fixed)
sim_ensemble <- apply(parvals, 1, function(p) sir_simulator$report(p)$value) ## 48 x 1000
sim_quantiles <- (t(apply(sim_ensemble, 1, quantile, c(0.025, 0.975)))
    |> as.data.frame()
    |> setNames(c("lwr_ens", "upr_ens"))
)
res2 <- cbind(res, sim_quantiles)
gg_ci2 <- (gg_base %+% res2) +
...
In this case the delta-method (dotted liens) and ensemble CIs (ribbons) are not that different (we can see that the ensemble CIs take care of the negative values in the CI); we certainly wouldn't be badly misled by the delta method CIs in this case.

**fixme**: 

- illustrate priors?
- illustrate fitting to incidence instead of prevalence?
- add more information from the [original article](https://www-ncbi-nlm-nih-gov/pmc/articles/PMC1603269/pdf/brmedj00115-0064.pdf) about probable starting date, duration, natural history?
- Comparison with [fitode](https://github.com/parksw3/fitode) ?
- is it worth trying 'non-statistical' objective functions (SSQ, chi-squared stat)? Extend `mk_calibrate` to support?
- add priors/regularization??
- add accumulator compartment and compute incidence
- examples of (statistical) diagnostics? e.g. compute residuals, plot vs. time or vs fitted values, scale-location plot, etc. ?

**fixme**: *intermediate* examples (simple time-varying parameters, irregularly sampled data, fitting prevalence/hospitalization/death, fitting multiple data streams ...) try to minimize 'boiler plate', focus on added features in each model ...
