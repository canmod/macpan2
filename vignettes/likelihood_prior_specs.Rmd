...
Distributional parameters have default parameter transformations inherited from their respective distribution. <!-- reference this once properly documented --> For example, standard deviations by definition are a strictly positive number, so the log transformation is convenient to use to ensure this condition is met.

Defaults can be changed by passing a distributional parameter transformation function `?macpan2::transform_distr_param` to the `trans` argument in `?macpan2::fit_distr_params` functions.
