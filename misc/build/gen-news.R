## mix content in `news-narratives.md` with automatically generated
## links to github diffs between versions

narratives = readLines("news-narratives.md")
version_heading = grep("^## [0-9]+\\.[0-9]+\\.[0-9]+", narratives)
versions_with_narratives = sub(
    "^## ([0-9]+\\.[0-9]+\\.[0-9]+)", "\\1"
  , narratives[version_heading]
)
narrative_list = mapply(
    \(start, end) narratives[start:end]
  , version_heading + 1
  , c(version_heading[-1] - 1, length(narratives))
  , SIMPLIFY = FALSE
) |> setNames(versions_with_narratives)
...
for (version in versions_all) {
  output_list[[version]] = c(
      sprintf("## Changes in %s", version), ""
    , just_links[[version]], ""
  )
  if (version %in% versions_with_narratives) {
    output_list[[version]] = c(
        output_list[[version]]
      , narrative_list[[version]]
    )
  }
}
