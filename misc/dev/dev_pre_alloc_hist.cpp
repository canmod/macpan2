...
vector<ListOfMatrices<Type> > MakeSimulationHistory(
    ...
    vector<ListOfMatrices<Type> > simulation_history(time_steps+2);//, hist_shape_template);
    for (int tt=0; tt<time_steps+2; tt++)
        simulation_history[tt] = hist_shape_template;
    //vector<ListOfMatrices<Type> >::iterator iter;
    //std::fill(iter.begin(), iter_end(), hist_shape_template);
...
void UpdateSimulationHistory(
    ...
    for (int i=0; i<mats_save_hist.size(); i++)
        if (mats_save_hist[i]<0)
            hist[t].m_matrices[i] = mats.m_matrices[i];
...
Type objective_function<Type>::operator() ()
{
    ...
    DATA_STRUCT(mats_shape, ListOfMatrices);
    ...
    vector<ListOfMatrices<Type> > simulation_history = MakeSimulationHistory(
        time_steps,
        mats_save_hist,
        mats_shape //hist_shape_template
    );
