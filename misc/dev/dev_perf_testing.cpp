...
struct ListOfMatrices {
    ...
    ListOfMatrices(SEXP RListOfMatrices){ // Constructor
        // Get elements by their indices
        int n = length(RListOfMatrices);
        vector<matrix<Type> > vs(n);
        m_matrices = vs;

        for (int i = 0; i < n; i++) {
            m_matrices[i] = asMatrix<Type>(VECTOR_ELT(RListOfMatrices, i));
        }
    }
...
public:
    ...
    matrix<Type> EvalExpr(
        ...
        const vector<int>& mats_saved_indices_inverse,
    ...
    )
    {
        ...
        switch (table_n[row]) {
            ...
            default:
                ...
                for (int i=0; i<n; i++) {
                    args[i] = EvalExpr(hist, t, mats_save_hist, mats_saved_indices_inverse, table_x, table_n, table_i, \
                ...
                switch(table_x[row]+1) {
                    ...
                    case MP2_RBIND_TIME:
                        ...
                        int rbind_length, nRows, nCols, savedMatIndex;
                        savedMatIndex = mats_saved_indices_inverse[matIndex];
                        ...
                        for (int i=0; i<timeIndex.size(); i++) {
                            ...
                            if (rowIndex<t && rowIndex>=lowerTimeBound) {
                                nRows = hist[rowIndex].m_matrices[savedMatIndex].rows();
                                nCols = hist[rowIndex].m_matrices[savedMatIndex].cols();
                            }
                            else if (rowIndex==t) {
                        ...
                        if (rbind_length>0) {
                            ...
                            for (int i=0; i<timeIndex.size(); i++) {
                                rowIndex = CppAD::Integer(timeIndex.coeff(i,0));
                                if (rowIndex<t && rowIndex>=lowerTimeBound) {
                                    if (hist[rowIndex].m_matrices[savedMatIndex].rows()!=0 &&
                                        hist[rowIndex].m_matrices[savedMatIndex].cols()!=0) {
                                        m.block(rbind_length*rows, 0, rows, cols) = hist[rowIndex].m_matrices[savedMatIndex];
                                        rbind_length++;
                                    }
                                }
                                else if (rowIndex==t) {
...
void UpdateSimulationHistory(
    ...
    const vector<int>& mats_save_hist,
    const vector<int>& mats_saved_indices,
    const vector<int>& mats_saved_indices_inverse,
    const ListOfMatrices<Type>& init_hist
) {
    matrix<Type> emptyMat;
    // std::cout << "IUYIUYIUYIUY" << mats[mats_saved_indices_inverse] << std::endl;
    ListOfMatrices<Type> ms(init_hist);
    for (int i=0; i<mats_saved_indices.size(); i++) {
        ms.m_matrices[i] = mats[mats_saved_indices_inverse[i]];
    }
    // if the history of the matrix is not to be saved,
    // just save an empty matrix to save space
...
Type objective_function<Type>::operator() ()
{
    ...
    // DATA_IVECTOR(mats_save_dims);
    DATA_STRUCT(init_hist, ListOfMatrices);
    ...
    vector<int> mats_saved_indices(mats_save_hist.sum());
    vector<int> mats_saved_indices_inverse(mats_save_hist.size());
    int matIndex = 0;
    for (int i=0; i<mats_save_hist.size(); i++) {
      if (mats_save_hist.coeff(i)) {
        mats_saved_indices.coeffRef(matIndex) = i;
        mats_saved_indices_inverse.coeffRef(i) = matIndex;
        matIndex++;
      } else {
        mats_saved_indices_inverse.coeffRef(i) = -1;
      }
    }
    std::cout << "indices of saved matrices:" << std::endl;
    std::cout << mats_saved_indices << std::endl;
    std::cout << mats_saved_indices_inverse <<std::endl;

    // 2 Replace some of elements of some matrices with parameters
    n = p_par_id.size();
    for (int i=0; i<n; i++)
        mats.m_matrices[p_mat_id[i]].coeffRef(
            p_row_id[i], p_col_id[i]
        ) = params[p_par_id[i]];

    n = r_par_id.size();
    for (int i=0; i<n; i++)
        mats.m_matrices[r_mat_id[i]].coeffRef(
            r_row_id[i], r_col_id[i]
        ) = random[r_par_id[i]];
