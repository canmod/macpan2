...
simulator$add$matrices(
  ...
  , noisy_I = empty_matrix
  , log_lik = empty_matrix
  , .mats_to_save = c("simulated_I", "total_inflow", "noisy_I")
  , .mats_to_return = c("noisy_I")
...
# simulator$insert$expressions(
#     poisson = simulated_I ~ rpois(simulated_I)
#   , .simulate_exprs = "poisson"
#   , .at = Inf
#   , .phase = "after"
# )
...
simulator$replace$params(
  default = c(log(0.1), qlogis(0.1)),
  mat = c("log_beta", "logit_gamma")
)

dd = simulator$ad_fun()
dd$simulate()$values[,5]

filter(simulator$simulate(), matrix == "noisy_I") %>% pull(value) %>% plot(las = 1)
filter(simulator$report(), matrix == "noisy_I") %>% pull(value) %>% plot(las = 1)
...
simulator$print$expressions()
t(apply(apply(mvrnorm(10, simulator$par.fixed(), simulator$cov.fixed()), 1, simulator$report_values), 1, quantile, probs = c(0.025, 0.5, 0.975)))
simulator$cov.fixed()
simulator$par.fixed()

View(simulator$report_ensemble(.phases = "during"))
## Step 6: use the engine object
simulator$optimize$optim()
sdreport(simulator$ad_fun())
...
simulator$current$params_frame()
simulator$replace$time_steps(200)
plot(obs_time_steps, observed_I)
lines(1:200, filter(simulator$report(.phases = "during"), matrix == "state", row == "I")$value, col = "red")
simulator$current$params_frame()
