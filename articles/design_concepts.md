...
![](../reference/figures//design-concepts.svg)
...
compartmental modelling (right). The major steps of information
...
2.  The structure of a compartmental model is defined in one of three
    ...
    - (2a) A model is chosen from a model library and read into `R`,
      optionally updating the model structure using an [engine-agnostic
      model specification
    ...
    - (2c) A model is written from scratch using one of the
      ...
      automatically executed and if 2b is chosen that 2c is automatic.
      The choice here is just how close (2c) or far (2a) from the actual
      ...
      taken, the output of step 2 is a model simulator that can be used
      to generate modelling outputs like simulated incidence time-series
      or reproduction numbers.
...
computing $\mathcal{R}_{0}$ in models with arbitrary complexity, whereas
...
One option is the same format as the output of `mp_trajectory`. This
...
- Easy to manipulate output into input for testing calibration
  functionality.
- Possibly simpler argument list to `mp_calibrate` because we would just
  relate the observed data to simulated data with the same name, of
  course we would still need an interface for distributional
  assumptions.
...
$i$th flow is from $z\lbrack i\rbrack$ to $y\lbrack i\rbrack$ at
per-capita rate $b\lbrack i\rbrack$.

The other way to think about it is that for a single flow, $x_{i}$ is
the from state, $x_{j}$ is the to state, and $b_{k}$ is the per-capita
flow rate.
