...
The values in these parameter vectors are used to update certain elements in the matrix-valued variables within `mats`. The elements to be updated are described on the R side by two data frames with one row for every matrix element to be replaced. These data frame are constructed in R and passed to C++. The columns for the data frame associated with `params` are the following.
...
* `DATA_IVECTOR(p_row_id)` -- indices of the rows within each matrix associated with an element to be replaced by parametes
...
After the input matrices in `mats` are updated using `params` and `random`, the these matrices can be modified. We refer to this process of modification as simulation. There are three phases to the simulation process.
...
* `DATA_IVECTOR(expr_output_count)`
    * Number of matrices modified by the expression
...
* `DATA_IVECTOR(expr_output_id)`
    * Index into `mats` identifying the first matrix produced by the expression
    * Matrices that are modified by the same function must be sequentially grouped together in `mats` (Implementation question: this is a genuine limitation and may be worth avoiding?)
...
* `DATA_IVECTOR(expr_sim_block)`
    ...
    * A simulation block is a set of expressions that are evaluated within a `SIMULATE` macro within TMB
    * Expressions in a simulation block must be adjacent with respect to all relevant orderings (`expr_ord_pre_sim`, `expr_ord_sim`, and `expr_ord_post_sim`)
    * A value of `-1` indicates that the expression is to be evaluated outside of a simulation block (we expect this to be the standard case)
...
It was briefly introduced in the section on [Expressions] that it is possible to modify several matrices using the same expression. This is achieved by choosing an appropriate function at the root of the parse tree. Such a function is special in that it would return a list of matrices instead of a single matrix. An example might be a function to return both the dominant eigenvector and eigenvalue of an input matrix.

In order to make lists of matrices a consistent concept, we need to introduce five additional special functions: `list`, `extract`, `select`, `map`, and `reduce`.
...
1. `l` -- A list of matrices
2. `i` -- A one-by-one matrix with a single integer

The return value of `extract` is the `i`th matrix in `l`.

The `select` function takes the following arguments.

1. `l` -- A list of matrices
2. `i` -- A matrix of integers

The return value of `select` is another list of matrices with the elements of `l` that are indexed by the elements of `i`. The output list is sorted in the (row-major) order in which they appear in `i`.

The `map` function takes the following arguments.

1. `l` -- A list of matrices
2. `f` -- A function that accepts one matrix and returns one matrix

The return value of `map` is another list with the results of applying `f` to each matrix in `l`.
...
1. `l` -- A list of matrices
2. `f` -- A function that accepts several matrices and returns one matrix

The return value of `reduce` is the matrix returned by passing all of the matrices in `l` to `f`
...
To be able to refer to past times for matrices that have their simulation history saved, we have four more special functions: `extract_time`, `extract_lag`, `select_time`, and `select_lag`.

The `extract_time` and `extract_lag` functions each take the following arguments.

1. `m` -- A matrix in `mats`
2. `i` -- A one-by-one matrix with a single integer

The return value of `extract_time` is the value of `m` at iteration `i`. The return value of `extract_lag` is similar but returns the value of `m` at iteration `t-i`.

The `select_time` and `select_lag` functions each take the following arguments.

1. `m` -- A matrix in `mats`
2. `i` -- A matrix of integers

The return value of `select_time` is a list of the values of `m` at each iteration given by the elements of `i`. The return value of `select_lag` is similar but returns a list of values at iterations given by `t-i`.

For all four of these functions, if `m` does not refer to a matrix that has a saved simulation history than an error is returned.
