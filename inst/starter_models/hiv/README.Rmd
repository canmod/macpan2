...
To illustrate calibration, we simulate 20 years of data from a population of `1e7` people using two variables: the prevalence of infectious individuals who are `treated` and `untreated`. We initialize with `1000` infectious individuals in each of the four disease stages, and zero untreated individuals.  We add Poisson noise to these variables. We also compute reported incidence using a reporting fraction of `0.1`. Although we do not use the resulting `reports` variable in this example, we include it to demonstrate how such a variable can be generated.
