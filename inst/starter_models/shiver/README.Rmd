...
```{r gharouni_parameters, include=FALSE}
# Gharouni parameters
...
#   )
# )
# testing out modifying hill instead of gharouni hack
hill_data = simple_sims(
    iteration_exprs = list(
        hill_fn ~ (A * (sc * (S^n)))/((half_max)^n + (sc * (S^n)))
      , vaccination ~ hill_fn/S
      #, flow ~ vaccination * S
      #, S ~ S - flow
    )
  , time_steps = 1L
