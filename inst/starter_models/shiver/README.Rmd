...
Our prior for `sigma` is similar to the posterior, but `gamma_h` seems to have been pushed up by the data from about `0.05` to about `1.11`. We still do not have confidence in our estimate of `p`. We now have five other parameters controlling transmission, and so to interpret them we really need a plot of how transmission varies over time in the model. We add this variable to our model fit plot.
