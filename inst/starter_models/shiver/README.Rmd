...
multi_traj_spec = (spec
  ...
  |> mp_tmb_insert(phase = "during", at = 1L, expressions = list(beta_s ~ beta_s * rbf_beta))
)
```

```{r multiple_traj_calib}
#off = 15
dd = rbind(reported_hospitalizations, reported_cases)#, data.frame(time = -off - 1, value = 0, matrix = "H")) |> mutate(time = time + off)
# calibrate
shiver_calibrator = mp_tmb_calibrator(
  ...
  , traj = list(H = mp_neg_bin(1), reported_incidence = mp_neg_bin(1))
  , par = c("log_beta_s","log_E", "log_sigma", "log_gamma_h", "logit_report_prob") 
  , tv = mp_rbf("rbf_beta", 2, sparse_tol = 1e-8, fit_prior_sd = FALSE, prior_sd = 0.01)
  #, par = c("log_beta", "logit_p", "log_E_I_ratio", "sigma", "incidence_report_prob")
  , outputs = c(states, "reported_incidence", "beta_s")
)
```
...
```{r}
tv_beta_spec = (multi_traj_spec
  |> mp_tmb_insert(
      phase = "during"
    , at = 1L
    , expressions = list(beta ~ sigma * sigma_rbf)
    , default = list(beta_rbf = 0)
  )
)
```

```{r}
tv_beta_calibrator = mp_tmb_calibrator(
    spec = tv_beta_spec
    # row bind both observed data
  , data = dd
    # fit both trajectories
  , traj = c("H","reported_incidence")
  , par = c("log_beta", "sigma", "log_E_I_ratio", "incidence_report_prob")
  , tv = mp_rbf("sigma_rbf", 4, initial_weights = rep(0, 4),  sparse_tol = 1e-8, fit_prior_sd = FALSE, prior_sd = 1e-2)
  , outputs = c(states, "reported_incidence", "beta")
)
```

```{r}
mp_optimize(tv_beta_calibrator)
```

```{r}
mp_tmb_coef(tv_beta_calibrator)
```
...
(tv_beta_calibrator
   |> mp_trajectory_sd(conf.int = TRUE)
   |> filter(matrix %in% c("H", "reported_incidence", "beta"))
   |> ggplot(aes(time, value))
   + geom_line(aes(y = value), colour = "red")
   + geom_ribbon(
        aes(ymin = conf.low, ymax = conf.high)
