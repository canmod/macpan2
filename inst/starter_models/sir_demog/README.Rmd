...
We modify the specification so that it is different from the default library model that we will calibrate. We will use the [?mp_rk4()] ODE solver.
```{r defaults}
...
We fit `beta` and `mu` to the simulated infection flow (i.e., incidence). To be consistent we use the [?mp_rk4()] ODE solver.
```{r calibrator}
