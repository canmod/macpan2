...
flow_rates = list(
  ...
  #HICUd? in variables.csv but not flows.csv
  #HICUs - shouldn't this be a flow? or does IsICUs contain this flow - do they skip H?
  , ICUd.D ~ ICUd * psi2
  , H.R ~ H * rho
)
...
state_updates = list(
  ...
  , D ~ D + ICUd.D # where is IsD? (severely infected that die before they get hospitalized)
)
