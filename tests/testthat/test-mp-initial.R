test_that("initial values in model specifications and model simulators are consistent up to row order", {
  ...
  expect_equal(initial_spec[order(initial_spec$matrix), , drop = FALSE],initial_sim[order(initial_sim$matrix), , drop = FALSE],ignore_attr = TRUE)
  expect_equal(mp_initial_list(sir_spec),mp_initial_list(sir_sim))
