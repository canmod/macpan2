test_that("dates can be turned into time-steps", {
  ...
  expect_error(
      Weekly(start = base_date - days(30), end = base_date)
    , regexp = "The following fractional time steps were found"
  )
  expect_error(
      Daily(start = base_date + days(2), end = base_date)
    , regexp = "The following times fell before the start time"
  )
  expect_error(
      Daily(start = base_date - hours(26), end = base_date)
    , regexp = "none of these are in the following list of valid classes"
  )
  
  good_daily = Daily(start = base_date - days(17), end = base_date)
  good_weekly = Weekly(start = base_date - weeks(10), end = base_date)
  ...
  expect_equal(
    ...
    , data.frame(time_step = 1:11, ending_time = as.character(today() - weeks(10:0)))
  )
  expect_equal(
